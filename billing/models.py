...
class BillingProfile(models.Model):
    ...
    def charge(self, booking_obj, card=None):
        return Charge.objects.do(self, booking_obj, card)
...
class ChargeManager(models.Manager):
    def do(self, billing_profile, booking_obj, card=None):  # Charge.objects.do()
        card_obj = card
        if card_obj is None:
        ...
        c = stripe.Charge.create(
              amount=int(booking_obj.total * 100),  # 39.19 --> 3919
              currency="usd",
              customer=billing_profile.customer_id,
              source=card_obj.stripe_id,
              metadata={"order_id": booking_obj.booking_id},
            )
